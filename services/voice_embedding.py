# services/voice_embedding.py
# ECAPA-TDNN using locally downloaded model (NO SYMLINKS)
# UPGRADED FROM FAULTY RESEMBLYZER TO PROVEN ECAPA-TDNN SYSTEM
...
def record_audio(duration=7.0, fs=16000):
...
def apply_l2_normalization(embedding):
    """Apply L2 normalization to any embedding vector"""
    embedding = np.array(embedding).flatten()
    norm = np.linalg.norm(embedding)
    if norm > 1e-8:  # Avoid division by zero
        return embedding / norm
    else:
        console.print(f"[yellow]⚠️  Embedding norm too small ({norm:.8f}), returning original[/yellow]")
        return embedding

def get_ecapa_embedding(audio_np):
    """Generate ECAPA-TDNN embedding from audio with L2 normalization"""
    try:
        ...
        if embedding_norm > 1e-8:  # Avoid division by zero
            ...
            console.print(f"[cyan]🔧 Applied L2 normalization (norm: {embedding_norm:.4f} → 1.0)[/cyan]")
        else:
            console.print(f"[yellow]⚠️  Embedding norm too small ({embedding_norm:.8f}), skipping L2 normalization[/yellow]")
...
def record_and_embed_three_times(duration_per_clip=7.0, user_id=None):
    ...
    # Average the embeddings and apply L2 normalization to final result
    averaged_embedding = np.mean(embeddings, axis=0)
    final_embedding = apply_l2_normalization(averaged_embedding)
    console.print("[bold green]🎉 VOICE REGISTRATION COMPLETE — ECAPA-TDNN[/bold green]")
    console.print(f"[cyan]Final L2-normalized embedding shape: {final_embedding.shape}[/cyan]")
...
def verify_voice_live(stored_emb, duration=7.0, threshold=0.72):
    """Verify voice using ECAPA-TDNN with L2 normalized embeddings"""
    ...
    # Generate live embedding (already L2 normalized)
    live_emb = get_ecapa_embedding(cleaned)
    if live_emb is None:
        console.print("[red]❌ Failed to generate embedding[/red]")
        return 0.0, False
    
    # Ensure both embeddings are 1D arrays and apply L2 normalization to stored embedding
    stored_emb_flat = np.array(stored_emb).flatten()
    live_emb_flat = live_emb.flatten()
    
    # Apply L2 normalization to stored embedding (in case it wasn't normalized during registration)
    stored_norm = np.linalg.norm(stored_emb_flat)
    if stored_norm > 1e-8:
        stored_emb_flat = stored_emb_flat / stored_norm
        console.print(f"[cyan]🔧 Normalized stored embedding (norm: {stored_norm:.4f} → 1.0)[/cyan]")
    
    # Cosine similarity (simplified since both are L2 normalized)
    score = np.dot(stored_emb_flat, live_emb_flat)
    
    passed = score >= threshold
    status = "✅ PASS" if passed else "❌ FAIL"
    console.print(f"[bold]ECAPA-TDNN L2-Normalized Score: {score:.1%} → {status}[/bold]")
    
    return score, passed

def verify_voice_live_flask(stored_emb, duration=7.0, threshold=0.72):
    """Flask version without Rich console output - L2 normalized embeddings"""
    raw_audio = record_audio(duration)
    cleaned = apply_vad(raw_audio)
    ...
    live_emb = get_ecapa_embedding(cleaned)  # Already L2-normalized
    if live_emb is None:
        return 0.0, False
...
def record_and_embed_one_clip_flask(duration=7.0):
    """Flask-compatible single clip recording for backward compatibility - L2 normalized"""
    print(f"\n🎤 Recording single clip ({duration}s)...")
    
    # Record audio
    raw_audio = record_audio(duration)
    print(f"✅ Recorded {len(raw_audio)} samples")
    ...
    print(f"✅ VAD extracted {len(cleaned)} speech samples")
    
    # Generate embedding (already L2-normalized)
    embedding = get_ecapa_embedding(cleaned)
    if embedding is None:
        print("❌ Failed to generate embedding")
        return None
    
    print(f"✅ Generated L2-normalized embedding: {embedding.shape}")
    return embedding

def get_voice_embedding(audio_data, fs=16000):
    """Backward compatibility function - returns L2-normalized embedding"""
    return get_ecapa_embedding(audio_data)  # Already L2-normalized

# Aliases for backward compatibility
get_embedding_from_wav = get_ecapa_embedding
...
try:
    ...
    console.print("[yellow]Run 'pip install speechbrain' if not installed![/yellow]")