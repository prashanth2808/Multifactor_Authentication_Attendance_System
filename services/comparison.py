...
def find_best_match(
    ...
    for user in users:
        ...
        # Validate embedding dimensions before computing similarity
        valid_embs = []
        for emb in user_embs:
            if len(emb) == len(query_embedding):  # Check dimension match
                valid_embs.append(emb)
            else:
                print(f"⚠️ Skipping invalid embedding for {name}: shape {emb.shape} vs expected {query_embedding.shape}")
        
        if not valid_embs:
            continue  # Skip this user entirely if no valid embeddings
            
        scores = [cosine_similarity(query_embedding, emb) for emb in valid_embs]
